...
- **Deadfold** and **uncounted hour** remain provisional field terms for a
  contaminated area and interval respectively.
...
by an engineered fold. It does not consume the universe, rewrite all history, or
spread through memory and written description.

Records about a rupture are not contagious. Physical traversal, transport of an
active shard condition, or continued attachment to affected fold infrastructure
may extend a boundary.
...
Objects and survivors crossing a mature boundary emerge in one externally
coherent state. Which state emerges is uncontrollable. Contradiction may remain
in provenance, memory, records, or relationships, but two incompatible copies
have never remained stable as independently exportable resources.
...
- route junctions and engineered spatial folds;
...
the mechanism of all interstellar routes.
...
- shut down or separate local route and fold infrastructure;
...
- **Premature boundary-lock:** interference causes the rupture to stabilize
  sooner rather than collapse.
- **Boundary substitution:** a disrupted shard condition reappears elsewhere or
  earlier in the incident.
- **False concordance:** instruments temporarily share one local chart and
  report that the rupture has ended.
- **Command-chain inclusion:** the affected interval expands backward through
  preparations, placing observers inside the target history.
- **Objective inversion:** the rupture denies the attacker's approach, preserves
  the intended target in a locally coherent refuge, or creates a traversable
  relation useful to the opposing side.
- **Unintended function:** an attempted denial produces what appears to be a
  corridor, enclosure, arrival site, or release event, sometimes accompanied by
  an associated presence.
- **Reconciliation loss:** an apparently successful evacuation produces an
  externally coherent survivor set different from the one expected.
- **Anchor transport:** a removed object carries an immature boundary condition
  into a safe site.
- **Strategic non-result:** a target is destroyed in one authenticated history
  and intact in another, denying occupation or verification without producing
  an ordinary victory.
...
Pretechnical traditions describe an uncounted interval, differently numbered or
angular visitors, and objects that arrived before their discovery. Their exact
language varies, but the sequence of motifs resembles contemporary incidence
too closely for later editors to dismiss.

The existence and dating of an early document can receive strong evidence even
when the presence described within it remains uncertain.

### First instrumented encounter

A route or astronomical observatory records negative-order detections and
several internally valid maps of one facility. Later scholars coin **tachyon
shard** to describe the inferred initiating signatures.

The anchor event should be placed at a frontier junction or isolated observatory
rather than at Issa. Issa already carries deliberately unresolved nonhuman and
Rzez possibilities.

### First military interpretation

A fortified site becomes unoccupiable during a conflict. Neither side can prove
it caused the event or benefited from it. **Epoch denial** enters doctrine as a
description of consequence.

The incident should frustrate ordinary strategic interpretation rather than
quietly reveal the presence's motive.

### Contemporary confirmation

Distributed exterior instruments record a bounded rupture and evidence
associated with one or more presences. This connects the old accounts to a
continuing phenomenon without establishing that the same being, instrument, or
purpose appears in every case.

The first full entry should choose one contemporary Grade A rupture rather than
building its mystery entirely from generalized rumor.
...
imitate the associated presence. Military planners model strategic patterns
...
demonstrated repeatable control, and every credible attempt caused an unintended
boundary, interval, survivor outcome, strategic result, or associated-presence
event. Possessing a suspected anchor is therefore not equivalent to possessing a
weapon; it is possession of an opportunity to begin an incident whose final
target may include its operator.

The concept creates:

- concordance surveyors who map rupture rims without entering;
- rimkeepers responsible for long quarantine;
- chronometric laboratories maintaining separated clocks and archives;
- rescue teams trained in single-chart doctrine;
- courts evaluating mutually authentic but incompatible evidence;
- offices recording people whose legal biographies no longer have one supported
  chronology;
- military units able to sever route infrastructure during incidence;
- fraudulent shard cults and genuine traditions descended from early witnesses;
- prohibited programs attempting controlled closure;
- memorials retaining every authenticated casualty list;
- and laws against transporting suspected anchors.

Strategic propaganda will claim that atrocities or defeats occurred only in a
discarded history. Route architecture may acquire sacrificial disconnects around
critical junctions. **Uncounted hour** may become a cultural term for an interval
whose reality is acknowledged but cannot be narrated once.
...
The concept also establishes that folded spacetime is physically manipulable.
It must not automatically establish how ordinary interstellar routes work. It
...
| Propagates through matter and infrastructure. | Establishes through shard conditions, folds, routes, and causal junctions. |
...
- routes and corridors as deliberately underdefined infrastructure;
...
- What contemporary Grade A rupture anchors the entry?
- What observations establish a rupture boundary?
- What intervention can disrupt incidence without becoming a universal defense?
...
- Which externally coherent state emerges when matter or a survivor crosses the
  final boundary, what does reconciliation cost, and what contradictory
  provenance survives?
- How long can one locally coherent refuge persist?
- Can a mature rupture contract naturally, or only change and release?
- What prevents exportable duplication and selectable future information?
- How is a shard distinguished from fraud, Rzez damage, or ordinary route
  instability?
- Does the first entry include one epochal case or reserve epochal closure as a
  strongly supported threat model?
- Which institution has authority over quarantine?
- Are **deadfold** and **uncounted hour** retained as shared terms or replaced?
