---
document_type: entity-index
entity_class: places
authorial_status: provisional
source_entries:
  - A-7.01
  - B-4.01
  - C-6.01
  - D-1.01
source_concepts:
  - the-unhidden
  - the-manifold
---

# Places

## Extracted anchor places

| Place | Current role in shared canon | Status |
| --- | --- | --- |
| [Valec](valec.md) | Destroyed industrial world and Year Zero of the Compact Calendar. | Provisional |
| [Kharos](kharos.md) | Designated home world; later demographic and civic center of an Earth-descended population first durably sheltered on Avara. | Provisional |
| [Issa](issa.md) | Frontier gas giant with an anomalously separated inner moon. | Provisional |
| [Apsis Range](apsis-range.md) | Remote, deliberately uninhabited external range volume used for The Rehearsal's unique full firing and now held under permanent exclusion. | Established; containing system withheld |

## Kelan–Ruva system register

The [Kelan–Ruva system](kelan-ruva-system.md) is the first star system with a
system-level physical model in shared canon.

| Body | Class | Current role | Status |
| --- | --- | --- | --- |
| **Kelan** | G4 V star | Daylight-dominant primary of the circumbinary system. | Provisional |
| **Ruva** | M3 V star | Red-dwarf companion whose motion produces the paired-sun synodic cycle. | Provisional |
| **Edras** | Rocky planet | Hot, airless innermost surviving planet. | Provisional |
| **Talra** | Sub-Neptune | Hot volatile-rich inner planet. | Provisional |
| [**Kharos**](kharos.md) | Rocky super-Earth | Later home world; high-gravity settlement, courts, protected dissent, rehabilitation, and public legal consequence. | Provisional |
| **Neral** | Rocky planet | Cold ice-border world with limited activity. | Provisional |
| **Vaskor** | Gas giant | Shapes the Serrin Belt and carries a large satellite system. | Provisional |
| **Istra** | Ice giant | Outermost major planet. | Provisional |
| [**Odran**](odran.md) | Moon | Barren inner moon; founding-era transfer ground and later separated material, chronometric, calorimetric, variance, and archival reference world. | Provisional |
| [**Avara**](avara.md) | Habitable moon | Oldest continuously occupied human refuge; sparsely settled partner world with low-gravity medicine, thermal industry, cooperative-control history, and managed life support. | Provisional |

## Named places retained in the Rzez entry

The following remain entry-local until reused:

- **Keth Aru:** abandoned salt and obsidian city associated with the black-wedge
  artifact tradition.
- **Orsa:** highland region of dry-stone fortifications and anti-imperial clans.
- **Deren Keep:** frontier fortress on the western grain road.
- **Kareth:** location of the dynastic ossuary destroyed during the War of Seven
  Heirs.
- **Serevan:** maritime city-state known for ceramic-composite ships.
- **Orel:** river and city served by the Seven Bridges.
- **Avel Taran:** militarized plateau kingdom.
- **Leth Var:** university city associated with biological engineering.
- **Meren:** post-Compact desert city lost above a buried research complex.

This index records spelling and role without prematurely inventing geography or
political relationships. See [`../setting-scope.md`](../setting-scope.md) before
assigning any entry-local place to a world or system.
